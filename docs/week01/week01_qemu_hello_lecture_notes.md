...
本周是《LoongArch 汇编语言》课程的入口周，安排为**一次 90 分钟连堂课**。学生刚系统学习过 C 语言，通常已经熟悉变量、函数、循环、指针这些概念，但还没有真正建立"CPU 到底在执行什么"的底层模型，也普遍没有操作系统原理和 Makefile 的背景知识。

本周不追求讲完整操作系统，也不追求掌握大量 LoongArch 指令。核心任务只有一个：让学生亲眼看到一个 LoongArch 裸机程序从启动汇编进入 C 函数，并通过 UART 在 QEMU 终端输出：
...
本周的教学评价重点不是"学生记住多少指令"，而是"学生是否建立了正确的启动链路"。
...
教师要提醒：当前 `master` 已包含第 2 周的 `.data/.bss` 检查代码。第一周课堂只看 Hello 链路，后续输出和 `clear_bss` 标注为"下一周内容"，具体说明放在收尾的"本周边界"里，不在这里展开。
...
如果学生问当前源码里为什么有 `clear_bss`，回答：这是第 2 周要讲的 `.bss` 初始化，第一周先知道它发生在进入 C 之前，细节下一周展开。
...
配合 PPT 里的"第 1 周最小运行链路"流程图，给出文字版逐步说明，帮学生把流程图里的 5 个节点拆成更细的 8 步：
...
如果当前 master 同时输出第 2 周检查内容（`data section ok` / `bss section cleared` / `week1-week2 check done`），说明这些是第 2 周内容，第一周课堂只验收 Hello 链路。未执行过 `make run`，就不能写"已通过"；工具链缺失时记录为"未执行"，并写明失败原因；测试报告必须包含：已执行、未执行、失败原因、下一步命令。
...
如果一开始直接上开发板，学生会同时遇到串口线、固件、加载地址、板级 UART 地址、供电、烧录等问题，容易掩盖本周真正要学的启动链路。先在 QEMU 跑通，是为了建立可回归的教学基线。

### 9.6 当前 master 为什么有第 2 周内容？

当前工程已经包含第 2 周的 `.data/.bss` 检查代码。第一周课堂只取最小主线：`_start -> kernel_main() -> printk() -> UART`。其他输出可以暂时说明为下一周内容，不作为第一周展开重点。
