...
本手册用于《LoongArch 汇编语言》第 1 周自学实验。目标不是写一个完整操作系统，而是把一个最小 miniOS 从第一条指令启动起来，并在 QEMU 串口中看到第一行输出。
...
本实验是整门课程的起点。后续的 `.data/.bss`、函数调用、异常、中断、系统调用和开发板迁移，都会建立在本周的启动路径之上。
...
当前 `master` 可能已经包含第 2 周 `.data/.bss` 检查代码，所以你还可能看到类似后续验证输出。第 1 周只验收 `Hello miniOS on LoongArch64` 这条启动输出链路；不要把 `.data/.bss` 当成本周必须解释的内容。
