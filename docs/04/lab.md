...
**问题 2.1：贴出你在 `lib/mem_fp.S` 里补全的那 3 行代码，以及 `make run` 输出这一行的截图**

**问题 2.2：输出的 `44` 是怎么来的？说明 `st.b` 为什么只截取加法结果的低 8 位，而不是报错或者自动扩宽 `base` 数组**

**问题 2.3：对照 `mem_byte_add3`（第 32–38 行——逻辑和你写的完全一样，只是基址寄存器从 `$a0` 换成了先 `move` 到 `$t0` 再用）：你的代码在"取数 → 相加 → 写回"这三步的顺序和寄存器使用上，跟它是否一致？如果不一致，差在哪里？**
