...
它是刻意改过寄存器名和偏移量的**抽象练习题**，仓库代码里搜不到，不用去
找——分析思路跟 `mem_copy_d` 里的 `ld.d $t0, $a0, 0` 完全一样，只是换了寄
存器名和偏移量：
...
1. 这条指令的动作是什么？搬运的数据宽度是几位？
2. 三个操作数（`$t1`、`$sp`、`16`）分别扮演什么角色？
3. 有效地址是多少？写出具体的计算式。
4. 执行后，哪个寄存器的值变了？哪些寄存器、哪些内存区域没变？
