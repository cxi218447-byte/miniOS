# 第 10 次课教师讲义：构建、链接与调试

> 技术编号：`10`　|　建议检查点：`10-build-debug`（与 `09-trap-irq` 同一提交，本课不新增源文件）  
> 合并原第 7 次课（Makefile/链接脚本/镜像结构）+ 原第 8 次课（GDB 调试与反汇编分析），
> 由 4 学时压缩为 2 学时：本课只保留“看懂构建链路”与“能用工具验证”两个最小闭环。
>
> **2026-09-07 更新**：本学期实际排课把本课移到第 10 次课，作为理论部分的收官——讲到这里时，
> 学生已具备 `memset/memcpy/strlen`（第 7 次课）、UART/`sys_write`（第 8 次课）、
